const DEFAULTS = { serverUrl: "http://127.0.0.1:8765", useLlm: true, llmModel: "", voice: "default", temperature: 0.8, exaggeration: 0.5, cfgWeight: 0.5, playbackRate: 1.0 };
...
async function loadSettings() {
  ...
  $("temperature").value = s.temperature; $("exaggeration").value = s.exaggeration; $("cfgWeight").value = s.cfgWeight; $("rate").value = s.playbackRate;
...
async function saveSettings() {
  const s = {
    ...
    temperature: +$("temperature").value, exaggeration: +$("exaggeration").value, cfgWeight: +$("cfgWeight").value, playbackRate: +$("rate").value,
  };
...
async function refreshHealth(s) {
  try {
    ...
    $("voice").innerHTML = v.voices.map(n => `<option value="${n}">${n}</option>`).join("");
  } catch (e) {
...
$("rate").oninput = async () => { await saveSettings(); send({ type: "set-rate", rate: +$("rate").value }); };
